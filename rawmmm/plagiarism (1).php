...
<footer class="bg-dark text-white text-center py-3">
    ...
    .then(data => {
        console.log("API Response:", data); // Debugging ke liye response check karo
        resultContainer.style.display = "block";

        if (!data || data.error) {
            resultText.innerHTML = "<span class='text-danger'>Error: " + (data.error || "Invalid response from server") + "</span>";
            return;
        }

        // Topics list extract karo
        let topics = data.topics.length > 0 ? data.topics.map(t => t.label).join(", ") : "No topics detected";

        // Plagiarism percentage from API response
        let plagiarismPercentage = data.plagiarism_percentage || 0;

        // Message generate karo
        let plagiarismMessage = `<span class='text-success'>Plagiarism Analysis Completed!</span><br>
                                 <strong>Detected Topics:</strong> ${topics}<br>
                                 <strong>Plagiarism Score:</strong> ${plagiarismPercentage}% `;

        if (plagiarismPercentage === 0) {
            plagiarismMessage += "(Completely unique) 🎉";
        } else if (plagiarismPercentage < 30) {
            plagiarismMessage += "(Mostly unique)";
        } else if (plagiarismPercentage < 60) {
            plagiarismMessage += "(Consider revising)";
        } else {
            plagiarismMessage += "(Needs significant changes 🚨)";
        }

        resultText.innerHTML = plagiarismMessage;
    })
    .catch(error => {
        console.error("Fetch Error:", error); // Debugging ke liye error console me show hoga
        resultContainer.style.display = "block";
